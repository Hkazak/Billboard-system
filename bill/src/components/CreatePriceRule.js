    ...
    return (
        <form className="create-new-price-rule-block" hidden={hide}>
            ...
            <div className="create-price-rule-general-information">
                ...
                <select className="price-rule-input"
                        onChange={e => setSurfaceId(surfaces[e.target.selectedIndex - 1].id)}>
                    <option>Вид поверхности</option>
                    {surfaces.map(e => <option key={e.id}>{e.surface}</option>)}
                </select>
                <select className="price-rule-input"
                        onChange={e => setBillboardType(billboardTypes[e.target.selectedIndex - 1])}>
                    <option>Тип экспонирования</option>
                    {billboardTypes.map(e => <option key={e}>{e}</option>)}
                </select>
                <input required type="number" className="price-rule-input" placeholder="Цена за 1м х 1м"
